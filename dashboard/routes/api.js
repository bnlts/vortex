...
// ── OWNER ID — hardcodé dans le code, invisible depuis l'extérieur
const OWNER_ID = '148914355625394176'; // <-- Remplace par ton ID Discord

function requireOwner(req, res, next) {
  const client  = global.client;
  const myId    = client?.user?.id || null;
  // Double vérification : session ET ID Discord correspondant
  if (!req.session?.isOwner && myId !== OWNER_ID) {
    return res.status(403).json({ error: 'Accès refusé' });
  }
  next();
}

// POST /api/creator/login — vérifie que le token connecté est bien le owner
router.post('/creator/login', (req, res) => {
  const client = global.client;
  const myId   = client?.user?.id || null;
  if (myId !== OWNER_ID) return res.status(403).json({ error: 'Accès refusé' });
...
router.get('/creator/status', (req, res) => {
  const client  = global.client;
  const myId    = client?.user?.id || null;
  const isOwner = myId === OWNER_ID;
  if (isOwner) req.session.isOwner = true;
  res.json({
    isOwner : isOwner || !!req.session?.isOwner,
  });
});
...
router.get('/vip/status', (req, res) => {
  const client = global.client;
  const userId = client?.user?.id;
  if (!userId) return res.json({ vip: false });
  const info = VipManager.getVipInfo(userId);
  res.json({ vip: !!info && !info.expired, info: info || null });
});
