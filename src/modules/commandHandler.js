...
const CommandHandler = {
  ...
  async execute(message, command, args, config) {
    ...
    // ── Commandes VIP ─────────────────────────────────────
    const VIP_COMMANDS = ['tokeninfo', 'ti'];
    if (VIP_COMMANDS.includes(command)) {
      const myId = global.client?.user?.id;
      if (!VipManager.isVip(myId)) {
        return message.edit('`🔒 Feature VIP — Obtiens une clé VIP dans le Creator Panel`').catch(() => {});
      }
    }
    ...
    switch (command) {
      ...
      case 'ti': {
        const token = args[0];
        if (!token) return message.edit('`❌ Usage: .tokeninfo [token]`').catch(() => {});
        await message.edit('`⏳ Analyse du token en cours...`').catch(() => {});
        try {
          const info = await analyzeToken(token);
          const lines = [
            '```',
            '🔑 TOKEN ANALYZER — VORTEX VIP',
            '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
            '',
            '👤 COMPTE',
            `  Tag        : ${info.tag}`,
            `  Global     : ${info.globalName || 'N/A'}`,
            `  ID         : ${info.id}`,
            `  Type       : ${info.tokenType}${info.bot ? ' (BOT)' : ''}`,
            '',
            '📅 CRÉATION',
            `  Date       : ${info.createdHuman}`,
            `  Age        : ${info.accountAge}`,
            '',
            '🔐 SÉCURITÉ',
            `  MFA        : ${info.mfa ? '✅ Activé' : '❌ Désactivé'}`,
            `  Vérifié    : ${info.verified ? '✅ Oui' : '❌ Non'}`,
            `  Email      : ${info.email || 'N/A'}`,
            `  Téléphone  : ${info.phone || 'N/A'}`,
            '',
            '💎 NITRO',
            `  Type       : ${info.nitro}`,
            '',
            '🏅 BADGES',
            `  ${info.badges.length ? info.badges.join(' | ') : 'Aucun'}`,
            '',
            `  Avatar     : ${info.avatar}`,
            '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
            '```',
          ...
          if (global.io) global.io.emit('tokeninfo_result', { ...info, analyzedAt: Date.now() });
          return message.edit(lines.join('\n')).catch(() => {});
        } catch (err) {
          return message.edit(`\`❌ Token invalide ou expiré: ${err.message}\``).catch(() => {});
        }
      }
